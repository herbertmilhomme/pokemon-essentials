...
public partial class TilemapRenderer {
	public static partial class TilesetWrapper {
		public const int TILESET_WIDTH        = SOURCE_TILE_WIDTH * TILESET_TILES_PER_ROW;
		...
		public void wrapTileset(originalbmp) {
			...
			if (width == TILESET_WIDTH && originalbmp.mega()) {
				...
				if (columns * TILESET_WIDTH > MAX_TEX_SIZE) {
					Debug.LogError($"Tileset is too long!\n\nSIZE: {originalbmp.height}px\nHARDWARE LIMIT: {MAX_TEX_SIZE}px\nBOOSTED LIMIT: {MAX_TEX_SIZE_BOOSTED}px");
					//throw new ArgumentException($"Tileset is too long!\n\nSIZE: {originalbmp.height}px\nHARDWARE LIMIT: {MAX_TEX_SIZE}px\nBOOSTED LIMIT: {MAX_TEX_SIZE_BOOSTED}px");
				}
				bmp = new Bitmap(TILESET_WIDTH * columns, MAX_TEX_SIZE);
				remainder = height % MAX_TEX_SIZE;
				if (remainder == 0) remainder = MAX_TEX_SIZE;
				for (int col = columns; col < columns; col++) { //for 'columns' times do => |col|
					srcrect = new Rect(0, col * MAX_TEX_SIZE, width, (col + 1 == columns) ? remainder : MAX_TEX_SIZE);
		...
		public void getWrappedRect(src_rect) {
			ret = new Rect(0, 0, 0, 0);
			col = (int)Math.Floor(src_rect.y / MAX_TEX_SIZE.to_f);
		...
		public void blitWrappedPixels(destX, destY, dest, src, srcrect) {
			...
			if (merge) {
				...
				dest.blt(destX, destY, src, new Rect(srcrect_mod.x, srcrect_mod.y, srcrect_mod.width, side[:a]));
				dest.blt(destX, destY + side[:a], src, new Rect(srcrect_mod.x + TILESET_WIDTH, 0, srcrect_mod.width, side[:b]));
			} else {
				dest.blt(destX, destY, src, srcrect_mod);
			}
		}

		public void stretchBlitWrappedPixels(destrect, dest, src, srcrect) {
			if (srcrect.y + srcrect.width < MAX_TEX_SIZE) {
				// Save the processing power
			...
			tmp = new Bitmap(srcrect.width, srcrect.height);
			blitWrappedPixels(0, 0, tmp, src, srcrect);
			dest.stretch_blt(destrect, tmp, new Rect(0, 0, srcrect.width, srcrect.height));
		}
	}
}
