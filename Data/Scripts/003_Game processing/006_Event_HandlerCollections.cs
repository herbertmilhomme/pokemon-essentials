...
public static partial class EventHandlers {
	...
	public void add(event, key, proc) {
		if (!@@events.has_key(event)) @@events[event] = new NamedEvent();
...
//===============================================================================
public static partial class MenuHandlers {
	@@handlers = new List<string>();

	#region Class Functions
	#endregion

	public void add(menu, option, hash) {
		if (!@@handlers.has_key(menu)) @@handlers[menu] = new HandlerHash();
		@@handlers[menu].add(option, hash);
	}

	public void remove(menu, option) {
		@@handlers[menu]&.remove(option);
	}

	public void clear(menu) {
		@@handlers[menu]&.clear;
	}

	public void each(menu) {
		if (!@@handlers.has_key(menu)) return;
		@@handlers[menu].each((option, hash) => yield option, hash);
	}

	public void each_available(menu, *args) {
		if (!@@handlers.has_key(menu)) return;
		options = @@handlers[menu];
		keys = options.keys;
		sorted_keys = keys.sort_by(option => options[option]["order"] || keys.index(option));
		foreach (var option in sorted_keys) { //'sorted_keys.each' do => |option|
			hash = options[option];
			if (hash["condition"] && !hash["condition"].call(*args)) continue;
	...
	public void call(menu, option, function, *args) {
		option_hash = @@handlers[menu][option];
		if (!option_hash || !option_hash[function]) return null;
