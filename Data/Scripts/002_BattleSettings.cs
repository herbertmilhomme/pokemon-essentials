...
public static partial class Settings {
	...
	public const bool RECALCULATE_TURN_ORDER_AFTER_MEGA_EVOLUTION = (MECHANICS_GENERATION >= 7)
	// Whether turn order is recalculated after a Pokémon's Speed stat changes.
	public const bool RECALCULATE_TURN_ORDER_AFTER_SPEED_CHANGES  = (MECHANICS_GENERATION >= 8)
	...
	public const bool ANY_HIGH_LEVEL_POKEMON_CAN_DISOBEY          = false;
	...
	public const bool FOREIGN_HIGH_LEVEL_POKEMON_CAN_DISOBEY      = true;
	...
	public const int NO_MEGA_EVOLUTION = 34;
	...
	public const bool MOVE_CATEGORY_PER_MOVE          = (MECHANICS_GENERATION >= 4)
	...
	public const bool NEW_CRITICAL_HIT_RATE_MECHANICS = (MECHANICS_GENERATION >= 6)
	...
	public const bool MORE_TYPE_EFFECTS               = (MECHANICS_GENERATION >= 6)
	// The minimum number of Gym Badges required to boost each stat of a player's
	// Pokémon by 1.1x, in battle only.
	public const int NUM_BADGES_BOOST_ATTACK  = (MECHANICS_GENERATION >= 4) ? 999 : 1
	public const int NUM_BADGES_BOOST_DEFENSE = (MECHANICS_GENERATION >= 4) ? 999 : 5
	public const int NUM_BADGES_BOOST_SPATK   = (MECHANICS_GENERATION >= 4) ? 999 : 7
	public const int NUM_BADGES_BOOST_SPDEF   = (MECHANICS_GENERATION >= 4) ? 999 : 7
	public const int NUM_BADGES_BOOST_SPEED   = (MECHANICS_GENERATION >= 4) ? 999 : 3
	...
	// instead.
	public const bool USE_SNOWSTORM_WEATHER_INSTEAD_OF_HAIL = (MECHANICS_GENERATION >= 9)
	// Whether weather caused by an ability lasts 5 rounds (true) or forever (false).
	public const bool FIXED_DURATION_WEATHER_FROM_ABILITY   = (MECHANICS_GENERATION >= 6)
	// Whether X items (X Attack, etc.) raise their stat by 2 stages (true) or 1
	// (false).
	public const bool X_STAT_ITEMS_RAISE_BY_TWO_STAGES      = (MECHANICS_GENERATION >= 7)
	// Whether some Poké Balls have catch rate multipliers from Gen 7 (true) or
	// from earlier generations (false).
	public const bool NEW_POKE_BALL_CATCH_RATES             = (MECHANICS_GENERATION >= 7)
	...
	public const bool SOUL_DEW_POWERS_UP_TYPES              = (MECHANICS_GENERATION >= 7)
	...
	// fainting, etc.
	public const bool AFFECTION_EFFECTS        = false;
	...
	public const int APPLY_HAPPINESS_SOFT_CAP = AFFECTION_EFFECTS;
	...
	public const bool ENABLE_CRITICAL_CAPTURES             = (MECHANICS_GENERATION >= 5)
	...
	// the Options screen.
	public const bool NEW_CAPTURE_CAN_REPLACE_PARTY_MEMBER = (MECHANICS_GENERATION >= 7)
	...
	// the gainer's level.
	public const bool SCALED_EXP_FORMULA            = (MECHANICS_GENERATION == 5 || MECHANICS_GENERATION >= 7)
	...
	public const bool SPLIT_EXP_BETWEEN_GAINERS     = (MECHANICS_GENERATION <= 5)
	// Whether the Exp gained from beating a Pokémon is multiplied by 1.5 if that
	// Pokémon is owned by another trainer.
	public const bool MORE_EXP_FROM_TRAINER_POKEMON = (MECHANICS_GENERATION <= 6)
	// Whether a Pokémon holding a Power item gains 8 (true) or 4 (false) EVs in
	// the relevant stat.
	public const bool MORE_EVS_FROM_POWER_ITEMS     = (MECHANICS_GENERATION >= 7)
	// Whether Pokémon gain Exp for capturing a Pokémon.
	public const bool GAIN_EXP_FOR_CAPTURE          = (MECHANICS_GENERATION >= 6)

	//-----------------------------------------------------------------------------
	// End of battle
	//-----------------------------------------------------------------------------

	public const bool CAN_FORFEIT_TRAINER_BATTLES         = (MECHANICS_GENERATION >= 9)
	// The Game Switch which, while ON, prevents the player from losing money if
	// they lose a battle (they can still gain money from trainers for winning).
	public const int NO_MONEY_LOSS                       = 33;
	...
	public const bool CHECK_EVOLUTION_AFTER_ALL_BATTLES   = (MECHANICS_GENERATION >= 6)
	// Whether fainted Pokémon can try to evolve after a battle.
	public const bool CHECK_EVOLUTION_FOR_FAINTED_POKEMON = true;
	...
	public const bool SMARTER_WILD_LEGENDARY_POKEMON = true;
}
